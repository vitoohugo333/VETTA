...
assert.ok(sw.includes('vetta-premium-ui-1'), 'cache deve mudar com a nova autoridade visual');
assert.ok(sw.includes('HAD_ACTIVE_WORKER'), 'primeira instalação e atualização devem permanecer estados distintos');
for (const asset of ['./ui/main.js','./ui/context.js','./ui/model.js','./ui/store.js','./ui/premium.css']) assert.ok(sw.includes(`'${asset}'`), `${asset} deve existir offline`);
