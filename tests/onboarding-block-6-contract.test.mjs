...
assert.match(model,/onboardingComplete: false/);
...
assert.match(main,/model\.state\.onboardingComplete=true/,'Conclusão deve usar o estado financeiro único.');
assert.match(main,/id:'r360-rental'[^]*kind:'weekly'/,'Carro alugado deve criar obrigação semanal explícita.');
assert.match(main,/ONBOARDING|saveOnboardingDraft|clearOnboardingDraft/,'Interrupção deve preservar rascunho sem criar outra base financeira.');
