...
assert.match(store, /if \(view === 'costs'\)[^]*planningSection: 'costs'/, 'A store deve traduzir Custos diretamente para a superfície financeira.');
assert.match(main, /ui\.primary\('costs'\)/, 'Ações contextuais devem abrir Custos pela store única.');
