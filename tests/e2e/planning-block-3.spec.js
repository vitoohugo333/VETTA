...
const initialState = {
  ...
  workWeekdays: [1, 2, 3, 4, 5, 6], extraDaysOff: 1, revenueKm: 2.25,
...
async function openSection(page, key) {
  ...
  if (secondary && !await page.locator('#planningSecondary').getAttribute('open')) await page.locator('#planningSecondary > summary').click();
...
test('Plano apresenta quatro decisões essenciais antes das análises', async ({ page }) => {
  ...
  const coreButtons = page.locator('#planningHub [data-planning-core] [data-planning-section-open]');
  await expect(coreButtons).toHaveCount(4);
  expect(await coreButtons.evaluateAll(nodes => nodes.map(node => node.dataset.planningSectionOpen))).toEqual(['goals', 'agenda', 'costs', 'operation']);
  const secondaryButtons = page.locator('#planningSecondary [data-planning-section-open]');
  await expect(secondaryButtons).toHaveCount(3);
  expect(await secondaryButtons.evaluateAll(nodes => nodes.map(node => node.dataset.planningSectionOpen))).toEqual(['distribution', 'learning', 'advanced']);
...
test('zerar a meta transforma Objetivo em parte incompleta do plano', async ({ page }) => {
  await openApp(page);
  await openPlan(page);
  await openSection(page, 'goals');
